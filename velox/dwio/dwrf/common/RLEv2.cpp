 ...
 */
template <bool isSigned>
int64_t* RleEncoderV2<isSigned>::prepareForDirectOrPatchedBase(EncodingOption& option) {
  if (isSigned) {
...
template int64_t* RleEncoderV2<true>::prepareForDirectOrPatchedBase(EncodingOption& option);
template int64_t* RleEncoderV2<false>::prepareForDirectOrPatchedBase(EncodingOption& option);
...
void RleEncoderV2<isSigned>::determineEncoding(EncodingOption& option) {
  ...
  if (option.min == max) {
    if (!option.isFixedDelta) {
      throw std::invalid_argument(std::to_string(option.min) + "==" +
  ...
  if (option.isFixedDelta) {
    if (currDelta != initialDelta) {
      throw std::invalid_argument("currDelta should be equal to initialDelta for fixed delta encoding");
    }
...
template void RleEncoderV2<false>::computeZigZagLiterals(EncodingOption &option);
...
template <bool isSigned>
void RleEncoderV2<isSigned>::writeInts(int64_t* input, uint32_t offset, size_t len, uint32_t bitSize) {
...
template void RleEncoderV2<true>::writeInts(int64_t* input, uint32_t offset, size_t len, uint32_t bitSize);
template void RleEncoderV2<false>::writeInts(int64_t* input, uint32_t offset, size_t len, uint32_t bitSize);
...
template void RleEncoderV2<true>::writePatchedBasedValues(EncodingOption& option);
template void RleEncoderV2<false>::writePatchedBasedValues(EncodingOption& option);
 ...
 */
template <bool isSigned>
uint32_t RleEncoderV2<isSigned>::percentileBits(int64_t* data, size_t offset, size_t length, double p, bool reuseHist) {
  if ((p > 1.0) || (p <= 0.0)) {
...
template uint32_t RleEncoderV2<true>::percentileBits(int64_t* data, size_t offset, size_t length, double p, bool reuseHist);
template uint32_t RleEncoderV2<false>::percentileBits(int64_t* data, size_t offset, size_t length, double p, bool reuseHist);
